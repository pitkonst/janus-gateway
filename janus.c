...
json_t *janus_admin_stream_summary(janus_ice_stream *stream) {
	...
	if(stream->audio_rtcp_ctx != NULL) {
		...
		json_object_set_new(audio_rtcp_stats, "lrr", json_integer(janus_rtcp_context_get_lrr(stream->audio_rtcp_ctx)));
	...
	for(vindex=0; vindex<3; vindex++) {
		if(stream->video_rtcp_ctx[vindex] != NULL) {
			...
			json_object_set_new(video_rtcp_stats, "lrr", json_integer(janus_rtcp_context_get_lrr(stream->video_rtcp_ctx[vindex])));
